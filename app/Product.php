...
class Product extends Model
{
	...
	public static function productCount($cat_id){
	 ...
	 public static function getCurrencyRates($price){
        ...
        foreach($getCurrencies as $currency){
            ...
            }else if($currency->currency_code == "CUSD"){
                $EUR_Rate = round($price/$currency->exchange_rate,2);
            }else if($currency->currency_code == "RM"){
                $EUR_Rate = round($price/$currency->exchange_rate,2);
            }
        }
	...
	public static function getProductStock($product_id,$product_size){
        $getProductStock = ProductsAttribute::select('stok')->where(['product_id'=>$product_id,'size'=>$product_size])->first();
        return $getProductStock->stok;
    }
